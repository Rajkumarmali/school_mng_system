...
export const updateExamToShowResult = (examId) => async (dispatch) => {
    dispatch({ type: UPDATE_EXAM_TO_SHOW_RESULT_REQUEST })
    try {
        const res = await fetch(`${BASE_API}/update/showResult/${examId}`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "Authorization": "Bearer " + localStorage.getItem("token")
        ...
        const data = await res.json()
        dispatch({ type: UPDATE_EXAM_TO_SHOW_RESULT_SUCCESS, payload: data })
    } catch (err) {
        dispatch({ type: UPDATE_EXAM_TO_SHOW_RESULT_FAILER, payload: err.message })
    }
}

export const generateExamResult = (examId) => async (dispatch) => {
    dispatch({ type: GENERATE_EXAM_RESULT_REQUEST })
    try {
        const res = await fetch(`${BASE_API}/generate/exam-result/${examId}`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "Authorization": "Bearer " + localStorage.getItem("token")
        ...
        const data = await res.json();
        dispatch({ type: GENERATE_EXAM_RESULT_SUCCESS, payload: data })
    } catch (err) {
        dispatch({ type: GENERATE_EXAM_RESULT_FAILER, payload: err.message })
    }
}

export const getExamResultOverview = (examId) => async (dispatch) => {
    dispatch({ type: GET_EXAM_RESULT_OVERVIEW_REQUEST })
    try {
        const res = await fetch(`${BASE_API}/get/exam-result-overview/${examId}`, {
            method: "GET",
            headers: {
                "Content-Type": "application/json",
                "Authorization": "Bearer " + localStorage.getItem("token")
        ...
        const data = await res.json();
        dispatch({ type: GET_EXAM_RESULT_OVERVIEW_SUCCESS, payload: data });
    } catch (err) {
        dispatch({ type: GET_EXAM_RESULT_OVERVIEW_FAILER, payload: err.message })
    }
}

export const getStudentExamResult = (studentExamId, pageNumber, pageSize) => async (dispatch) => {
    dispatch({ type: GET_STUDENT_EXAM_RESULT_REQUEST })
    try {
        const res = await fetch(`${BASE_API}/get/student-exam-result/${studentExamId}?pageNumber=${pageNumber - 1}&pageSize=${pageSize}`, {
            method: "GET",
            headers: {
                "Content-Type": "application/json",
                "Authorization": "Bearer " + localStorage.getItem("token")
        ...
        const data = await res.json();
        dispatch({ type: GET_STUDENT_EXAM_RESULT_SUCCESS, payload: data })
    } catch (err) {
        dispatch({ type: GET_STUDENT_EXAM_RESULT_FAILER, payload: err.message })
    }
}